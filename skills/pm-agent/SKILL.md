...
> Task platform: GitLab (self-hosted) or GitHub, configured via `GIT_PROVIDER` in `.env`
...
2. Read `team.yaml` at project root → find the member where `username` matches the active USERNAME. Use `display_name` and `role` for this session.
3. If `.env` is missing or USERNAME is blank → ask once. Suggest copying `.env.example` to `.env` and filling it in.
...
3. **Source citations are mandatory.** Every PRD requirement carries an inline citation: `[RFC-XXX §N]`, `[Grooming YYYY-MM-DD]`, `[Kickoff YYYY-MM-DD]`, or `[Intake YYYY-MM-DD]` (scratch PRDs).
...
6. **Pipeline gates.** Sources/Intake → PRD → Review → Breakdown → Review → GitLab Issues. Never skip a gate; never auto-promote.
...
- Issues API (GitLab or GitHub) — current board state
- Intake interview answers — when no source documents exist (scratch PRD path)

### Outputs you produce
- `prd/<feature-slug>.md` — generated, versioned
- `breakdown/<feature-slug>_v<N>.yaml` — YAML, archived after approval
- Issues — created/updated via API (replaces XLSX)
...
| "Generate PRD for <feature>" | PRD pipeline Stage 1 (Path A) — or invoke `/pm-prd <feature>` |
| "PRD from scratch" / "no RFC" / "no documents" | PRD pipeline Stage 1 (Path B) — guided intake interview — invoke `/pm-prd <feature>` |
...
| "Apply the breakdown" / "Create issues" | Breakdown Stage 4 — archive YAML, create issues via API |
...
| 3 | Agent | Create MR/PR targeting **`develop`** (confirm with user first) |
| 4 | Team | Review MR/PR → merge into `develop` |
| 5 | QA | Test / validate PM artifacts on `develop` (Dev Release) |
| 6 | PM/PO | Open separate MR/PR: `develop` → `main` to lock artifacts |
...
- Inventing PRD content not traceable to source documents or recorded intake answers
